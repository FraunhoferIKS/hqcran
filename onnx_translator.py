"""
  ...
  reference: https://github.com/eth-sri/eran
...
def nchw_to_nhwc_shape(shape):
    """
    Reorders dimensions of a 1D array from NCHW to NHWC, since ONNX uses NCHW.
...
def nchw_to_nhwc_index(index: int) -> int:
    """
    Converts an single index from NCHW to NHWC, since ONNX uses NCHW.
        ...
        for node_idx, node in enumerate(self.nodes):
            ...
            if extract_norm and node_idx <= stop_norm_layer and len(operation_types) == 2 and node.op_type in ["Add", "Sub", "Mul", "Div"] and node.output[0] not in self.constants_map:
                ...
                if node.op_type == "Add":
                    ...
                    #print(f"Mean of {mean} extracted from network")
                elif node.op_type == "Sub":
                    mean = constant
                    #print(f"Mean of {mean} extracted from network")
                elif node.op_type == "Mul":
                    std = np.divide(1, constant)
                    #print(f"Std of {std} extracted from network")
                elif node.op_type == "Div":
                    std = constant
                    #print(f"Std of {std} extracted from network")
